...
старте выпускается самоподписанный сертификат на адрес, указанный в `-a`, и на
адрес обратной петли; сертификат живёт в памяти процесса, файлы ключей сервису
не нужны:

```
go run ./cmd/shortener -s -a localhost:8443 -b https://localhost:8443
```

Если `-b` не задан, базовый адрес коротких ссылок тоже становится `https://`:
иначе сервис выдавал бы ссылки, по которым сам не отвечает. Кука аутентификации
...
Сервис завершается штатно по `SIGTERM`, `SIGINT` и `SIGQUIT`. Сначала
`http.Server.Shutdown` дожидается уже начатых запросов, затем дочищаются
отложенные операции: воркер удаления применяет накопленные пачки, аудит
дорассылает принятые события, файловое хранилище закрывается.
